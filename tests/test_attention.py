...
class TestReshapeForBroadcast:
    def test_broadcast_shape(self, attention):
...
class TestApplyRotaryEmb:
    def test_output_shapes_match_input(self, attention):
    ...
    def test_preserves_dtype(self, attention):
...
class TestRepeatKv:
    def test_no_repeat_when_n_rep_is_one(self, attention):
        x = torch.randn(2, 10, 4, 32)
        out = attention.repeat_kv(x, n_rep=1)
        assert torch.equal(out, x)

    def test_repeat_expands_head_dim(self, attention):
        x = torch.randn(2, 10, 2, 32)
        n_rep = 4
        out = attention.repeat_kv(x, n_rep=n_rep)
        assert out.shape == (2, 10, 8, 32)

    def test_repeated_heads_are_identical(self, attention):
