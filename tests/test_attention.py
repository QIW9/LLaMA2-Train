...
class TestForward:
    def test_output_shape(self, attention: Attention) -> None:
        config = ModelConfig()
        batch_size = 1
        seq_len = 50
        dim = config.dim
        ...
        freqs_cos, freqs_sin = attention.precompute_freqs_cis(head_dim, seq_len)

        output = attention(x, freqs_cos, freqs_sin)

        assert output.shape == (batch_size, seq_len, dim)

    def test_output_dtype_matches_input(self, attention: Attention) -> None:
        config = ModelConfig()
        x = torch.rand(2, 16, config.dim, dtype=torch.float32)
        head_dim = config.dim // config.n_heads
        freqs_cos, freqs_sin = attention.precompute_freqs_cis(head_dim, 16)
